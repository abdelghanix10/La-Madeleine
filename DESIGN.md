...
- **Density**: _Artisanal Balanced_ (4/10) — generous whitespace, comfortable reading measures (max 65ch), no cluttered information stuffing.
- **Variance**: _Offset Asymmetric_ (7/10) — alternating text/media focal points, curved organic wave geometry in navigation, asymmetric card grids.
- **Motion Intensity**: _Fluid Spring_ (6/10) — physics-driven motion (`stiffness: 120, damping: 20`), scroll-triggered reveals, smooth drawer transitions with SVG dynamic path morphing.
...
- **Script & Heritage Eyebrows**: `Alex Brush` / `Pinyon Script` — Used sparingly for romantic Parisian eyebrows ("Since 2018", "Fait Maison", "Notre Histoire").
...
- ❌ No duplicate badges or redundant "Since 2018" overlays within the same section.
